...
#klase
imena = c("majica/top", "gaÄ‡e", "pulover", "haljina", "kaput", "sandale", "majica", "patike", "torba", "Äizme")
imena[train_oznake[5]+1]#ide +1 jer oznake počinju s 0

#normalizacija podataka - (x-mean)/std - ova formula se koristi kada psotoje heterogeni podaci
...
#sekvencijalni model se radi prilikon izrade kompleksnih mreža (koje se sastoje od viÅ¡e istih), sekvencijalni za običnu 
...
#sparse categorical crossentropy - koristi se kada imamo viÅ¡e od 2 klase i objekt pripada iskljuÄivo jednoj od tih
#categorical crossentropy - kad imamo viÅ¡e klasa i objekt moÅ¾e pripasti viÅ¡e od jedne
...
predikcija[1,]
which.max(predikcija[1,])#najveÄ‡a vjerojatnost klase
...
#definiranje inputa
input = layer_input(shape = dim(train_data)[2])#2 zato Å¡ta to predstavlja broj varijabli
#output sloj
predikcije = input %>%
...
filepath = file.path(checkpoint_dir, "Epoch-{epoch:02d}.hdf5")#svaka epoha zvat Ä‡e se epoch - 01,02,...
cp_callback = callback_model_checkpoint(filepath = filepath)
rm(model_f)
k_clear_session() #cisti memoriju, poÄinje bez oznaÄenih teÅ¾ina
...
#stopiranje kad doÄ‘emo do najboljeg modela
...
#gledamo razliku izmeÄ‘u kvartala za svaku varijablu kako bi identificirali outliere i skewness
#kako bi saznali Å¡ta nije u redu tribamo plotat varijable
hist(df$crime_rate)
pairs(~df$price+df$crime+df$n_hot_rooms+df$rainfall)

#barplotovi kategoriÄkih varijabli
...
#dummy varijable - za inkorporaciju kategoričkih varijabli u regresiju
#to je ono poseban stupac za svaku kategoriju
#ne postoji hijerarhija u ovim varijablama, ne mogu se kodirati u brojevima jer se oni mogu poredati po veličini
...
#ako nađemo neovisne vrijable koje su visoko kolerirane, tribamo jednu izbacit da izbjegnemo multikolinearnost
